...
Each installs its own `_extensions/<dept>/` folder. If you commit this
to version control collaborators can render the same output without the
package installed.
...
colour is, so the header stays readable whatever you pick. The colour
must be supplied as a correctly formatted HEX code.
...
When you set you theme the package will prompt you on how to modify the
YAML.
...
By default the logo doubles as the favicon. To use a **dedicated**
favicon instead, pass `favicon =`:
...
`favicon.png` shipped with the department (if it has one), then the
logo. A department without a bundled favicon simply uses its logo so
bundling one is optional.
